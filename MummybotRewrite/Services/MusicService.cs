namespace Mummybot.Services
{
    ...
    public class MusicService : BaseService
    {
        ...
        public MusicService(LogService log, IServiceProvider services)
        {
            LogService = log;
        ...
        public async Task<VolumeResult> SetVolumeAsync(ulong guildid, ushort volume)
        {
            if (ConnectedChannels.TryGetValue(guildid, out var musicDetails))
            {
                ...
                await (musicDetails.SendMessage($"Set volume to {volume}") ?? Task.CompletedTask);
            ...
            if (ConnectedChannels.TryGetValue(guildid, out var musicDetails))
            {
                ...
                if (result.Tracks.Count == 1)
                {
                    if (musicDetails.Player.IsPaused)
                    {
                        ...
                        await (musicDetails.SendMessage($"added {result.Tracks.First().Title} to position {musicDetails.Player.GetQueue().Count}") ?? Task.CompletedTask);
                    ...
                    else
                    {
                        ...
                        await (musicDetails.SendMessage($"now playing {result.Tracks.First().Title} <{result.Tracks.First().Url}>") ?? Task.CompletedTask);
                ...
                if (result.Tracks.Count > 1)
                {
                    if (canplaylist)
                    {
                        ...
                        if (!musicDetails.Player.IsPaused)
                        {
                            ...
                            await (musicDetails.SendMessage($"added {result.Tracks.Count} to the queue postions {firstposition}-{lastposition}") ?? Task.CompletedTask);
                        ...
                        else
                        {
                            ...
                            await (musicDetails.SendMessage($"now playing {qtrack.Title} <{qtrack.Url}> \n and added {result.Tracks.Count} to queue.") ?? Task.CompletedTask);
                            return new PlayResult()
                            {
                                IsSuccess = true,
                                isPlayList = true,
                    ...
                    await musicDetails.SendMessage("you arent whitelisted to request playlists");
        ...
        private async Task LavaNode_OnTrackEnd(TrackEndEventArg eventargs)
        {
            ...
            if (ConnectedChannels.TryGetValue(eventargs.GuildId, out musicDetails))
            {
                ...
                {
                    await musicDetails.SendMessage("There are no more items left in queue.");
                    LogService.LogInformation("There are no more items left in queue.", LogSource.MusicService, musicDetails.GuildID);
                    return;
                }
                LogService.LogInformation($"playing {item1.Title}", LogSource.MusicService, musicDetails.GuildID);
                await musicDetails.Player.PlayAsync(_lavaNode, item1);
                if (musicDetails.TextChannel is null)
                    return;
                else
                    await musicDetails.SendMessage($"Now playing: {item1.Title}");
            }
        }
    ...
    {
        public async Task SendMessage(string mesg)
        {
            if (TextChannel == null)
            {
                return;
                ...
                await TextChannel.SendMessageAsync(mesg);
                    }

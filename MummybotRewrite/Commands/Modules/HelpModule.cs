    ...
    public class HelpModule : MummyBase
    {
        ...
        public async Task HelpAsync()
        {
            try
            {
                ...
                foreach (var module in _commands.GetAllModules())
                {
                    ...
                    foreach (var cmd in module.Commands)
                    {
                        ...
                        if (result.IsSuccessful)
                            description += $"{prefixes.First()}{cmd.FullAliases.First()}\n";
                    }
                ...
                {
                    Pages = pages,
                    Author = new EmbedAuthorBuilder() { Name = Context.User.GetName(), IconUrl = Context.User.GetAvatarUrl() ?? Context.User.GetDefaultAvatarUrl() },
        ...
        public async Task HelpAsync([Description("Command you want the help for")]Module module)
        {
            ...
            builder.WithAuthor(Context.User.GetName(), Context.User.GetAvatarUrl());
            StringBuilder sb = new StringBuilder();
            foreach (var cmd in module.Commands)
            {
